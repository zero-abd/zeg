...
_RULES: List[Tuple[str, str, str]] = [
    ...
    ("origin", r"\bwhere are you (really |originally )?from\b|\bwhat('s| is) your (nationality|ethnicity|race|native language|mother tongue)\b|\bwhere were you born\b|\byour accent\b",
     "National origin, ethnicity and proxies for them."),
    ("citizenship", r"\b(are you an? )?(citizen|permanent resident)\b|\bgreen card\b|\bvisa (status|type|category)\b|\bwhat (kind of )?visa\b|\bsponsorship\b",
    ...
    ("health", r"\bdisabilit\w*\b|\bmedical (condition|history|issues?)\b|\bhealth (condition|issues?|problems?)\b|\bare you (on|taking) (any )?medication\b|\bmental health\b|\bhow many (sick )?days (have you|did you) (taken?|miss)\b",
    ...
    ("salary_history", r"\b(current|previous|last|prior) (salary|compensation|pay|wage)\b|\bhow much (do|did) you (make|earn)\b|\bwhat (are|were) you (currently )?(making|earning|paid)\b|\byour salary (is|was)\b",
